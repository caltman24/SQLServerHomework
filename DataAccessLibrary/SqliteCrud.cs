...
namespace DataAccessLibrary
{
    public class SqliteCrud : ISQLCrud
    {
        ...
        private readonly ISQLDataAccess _db = new SqliteDataAccess();
        ...
        {
            string sql = @"select p.Id, p.FirstName, p.LastName from People p";
        ...
        {
            string sql = @"select p.Id, p.FirstName, p.LastName from People p where p.Id = @Id";
        ...
        public void CreatePerson(PersonModel person)
        {
            ...
            {
                string sql = @"insert into Employers (EmployerName) values (@EmployerName);";
                _db.SaveData(sql, person.Employer, _connectionString);
                // Get back id

                sql = @"select e.Id from Employers e where e.EmployerName = @EmployerName;";
            ...
            if (person.Address?.Id <= 0)
            {
                ...
                sql = @"select a.Id from Addresses a where City = @City and State = @State and ZipCode = @ZipCode;";
